...
public class EntitiesTests
{
    ...
    [OneTimeSetUp]
    public void BeforeEverything() => _context = CreateDbContext();

    [OneTimeTearDown]
    public void AfterEverything() => _context = null;

    private static IBlogContext CreateDbContext(int blogCount = 25, int postCount = 10)
    {
        var context = new FakeBlogContext();

        var rnd = new Random(1);

        for (int i = 1; i <= blogCount; i++)
        {
            var blog = new Blog(i, $"Blog{i}");

            context.Blogs.Add(blog);

            for (int j = 1; j <= postCount; j++)
            {
                var post = new Post(i * 100 + j, $"Blog {blog.BlogId} - Post {j}", "My Content", blog,
                    DateTime.Today.AddDays(-rnd.Next(100)).AddSeconds(rnd.Next(30_000)), rnd.Next(500)
                    );
