...
namespace AdhocLinq
{
    internal partial class ExpressionParser
    {
        ...
        private readonly LambdaStore _λStore = new LambdaStore();
...
#pragma warning restore 0219
                ...
                else if (typeof(ITuple).IsAssignableFrom(left.Type) && typeof(ITuple).IsAssignableFrom(right.Type))
                {
                    switch (op.Id)
                    {
                        ...
                        case TokenId.LessThanEqual:
                            ...
                            {
                                case TokenId.GreaterThan:
                                    @operator = ExpressionType.GreaterThan; break;
                                case TokenId.GreaterThanEqual:
                                    @operator = ExpressionType.GreaterThanOrEqual; break;
                                case TokenId.LessThan:
                                    @operator = ExpressionType.LessThan; break;
                                case TokenId.LessThanEqual:
                                    @operator = ExpressionType.LessThanOrEqual; break;
        ...
        Expression ParsePrimaryStart()
        {
            switch (_token.Id)
            {
                ...
                default:
                    throw ParsingError(_token.Pos, "Expression expected");
            }
        }
        ...
        private Expression ParseTextExpression(string text)
        {
            switch (text)
            {
                case KEYWORD_IT: return ParseIt();
                case KEYWORD_PARENT: return ParseParent();
                case KEYWORD_ROOT: return ParseRoot();
                case SYMBOL_IT: return ParseIt();
                case SYMBOL_PARENT: return ParseParent();
                case SYMBOL_ROOT: return ParseRoot();
                case KEYWORD_IIF: return ParseIif();
                case KEYWORD_NEW: return ParseNew();
                case KEYWORD_TUPLE: return ParseTuple();
                default:
                    throw ParsingError(_token.Pos, $"'{text}' is not supported as textual entry");
            }
        }
        ...
        Expression ParseMemberAccess(Type type, Expression instance)
        {
            ...
            else
            {
                ...
                switch (member)
                {
                    case null: throw ParsingError(errorPos, $"No property or field '{id}' exists in type '{type.GetTypeName()}'");
                    case PropertyInfo property: return Expression.Property(instance, property);
                    case FieldInfo field: return Expression.Field(instance, field);
                    default: throw new NotSupportedException($"{type}.{id} member (of type {member.GetType().Name}) is not supported");
                }
            }
        }
        ...
        Expression ParseElementAccess(Expression expr)
        {
            ...
            else
            {
                ...
                {
                    case 0:
                        throw ParsingError(errorPos, $"No applicable indexer exists in type '{expr.Type.GetTypeName()}'");
                    case 1:
                        return Expression.Call(expr, (MethodInfo)mb, args);
                    default:
                        throw ParsingError(errorPos, $"Ambiguous invocation of indexer in type '{expr.Type.GetTypeName()}'");
                }
            }
        }
